...
export class SessionSummaryManager {
  ...
  _buildConversationText(messages) {
    ...
    for (const msg of messages) {
      ...
      if (msg.role === "assistant" && processed.length > ASSISTANT_CAP) {
        // 助手长回复只保留开头，摘要只需知道做了什么
        processed = processed.slice(0, ASSISTANT_CAP) + "…（长回复已截断）";
      }
  ...
  async rollingSummary(sessionId, messages, configPath, utilityModel) {
    ...
    let newSummary = await this._callRollingLLM(convText, prevSummary, configPath, utilityModel, turnCount);
   ...
   */
  async _callRollingLLM(convText, prevSummary, configPath, utilityModel, turnCount = 10) {
